...
- [ ] T003 Create `python/pyproject.toml`:
  ...
  - **`[tool.hatch.version]`**: `source = "custom"`, `path = "hatch_build.py"`. **`[tool.hatch.build.hooks.custom]`**: `path = "hatch_build.py"`.
  - **Targets**:
...
- [ ] T004 Create `python/hatch_build.py` (research R4, R6, R9), with a module docstring stating why it exists.
  ...
  - **`CustomMetadataHook`** / **version source**: implement a hatchling version source plugin class `CustomSource(VersionSourceInterface)` with `get_version_data()`:
    - with a repository root, read `VERSION`, strip it, and return `{"version": str(packaging.version.Version(raw))}`, so `1.4.0-dev.2` becomes `1.4.0.dev2`. Raise `ValueError` naming the file if PEP 440 rejects it;
    - without one, read `__version__` from `src/persian_text_guard/_version.py`, and fail with a clear message if the file is missing.
  - **`CustomBuildHook(BuildHookInterface)`**, `initialize()`:
    ...
    - copy the root `LICENSE` and `THIRD-PARTY-NOTICES.md` into `python/` (git-ignored), so `license-files` finds them. If hatchling reads `license-files` before build hooks run (check with a first `uv build`), do this copy in the version source instead, which runs during metadata. Record which one was needed in `verification.md`;
...
- [ ] T010 Fill and verify:
  1. `dotnet run --project dotnet/tools/PersianTextGuard.CorpusFill` prints "Filled 8 case(s); 0 disagreement(s)".
  2. **Review the diff.** The first seven must equal research R2's table, with positions in code points: for example, `kir𠀀` records `start` 0 and `length` 4, and censors to `****`. The eighth is new, so record what .NET does. If any of the first seven differs from R2, stop and report it: it would mean the JavaScript port and .NET disagree outside the corpus.
  3. Run `dotnet test dotnet/tests/PersianTextGuard.Conformance` (530 × 3, which is 521 cases plus 9 guards), and in `js/` run `npm run corpus` (528, which is 521 cases plus 7 guards). If the JavaScript port fails a new case, stop and report the case id and difference to the user. Fixing JavaScript is in scope, but only after their decision.
...
- [ ] T031 [US1] Create `python/scripts/check_consumers.py`:
  ...
  2. installs `dist/*.whl` into the first and `dist/*.tar.gz` into the second with `pip install --no-deps --no-index`;
  ...
  It prints 4 of 4 checks: wheel smoke, sdist smoke, typed usage and type errors (SC-007).
...
**Checkpoint**: The package installs from a wheel and an sdist, and works and type-checks for users (MVP).

---

## Phase 4: User Story 2 - The same answer as every other port, proven by the corpus (Priority: P1)

**Goal**: All 521 corpus cases pass on every supported Python, the bundled lists match, and one filter is safe across threads.
...
- [ ] T037 [US2] Run `uv run pytest -m corpus`: every case passes (521 cases plus the guards).
...
- [ ] T042 [US3] Add the Python jobs to `.github/workflows/ci.yml`, leaving every existing job name unchanged (research R13, contracts → "CI"):
...
- [ ] T047 [US3] 👤 Tell the user the two account steps from [contracts/package-and-release.md](contracts/package-and-release.md) → "Release 1.4.0", steps 1 and 2 (the PyPI pending publisher and the GitHub `pypi` environment), with the exact values. They are needed before the real release (T066), not before the dry run. Continue without waiting. Before T066, confirm the environment exists with `gh api repos/AmirehsanK/PersianTextGuard/environments/pypi`.
...
- [ ] T051 [US4] Create `python/tests/test_readme.py`. It reads `python/README.md`, extracts every fenced `python` block, and runs each in a fresh namespace with `exec`, as one parametrized test per block, with ids `block-<n>-line-<line>`. A block that raises fails its test with the block's line number. It also asserts at least 8 blocks, so an extraction bug cannot pass vacuously.
...
- [ ] T054 [US4] Update the root `README.md` (FR-025):
  - the packages section lists PyPI next to NuGet and npm, with `pip install persian-text-guard` and a two-line Python example;
...
- [ ] T059 Prepare the real CI dry run (research R19, SC-009), following 003's T067 with these changes:
  1. Commit and push `004-python-port`, with no pull request yet.
  2. Run `git switch -c dryrun/release-gates`.
  3. Edit `.github/workflows/ci.yml` only:
  ...
  4. Set `VERSION` to `1.4.0-dev.1`.
  5. **Safety check** before committing: `grep -nE "dotnet nuget push|NuGet/login|gh-action-pypi-publish" .github/workflows/ci.yml` prints nothing, and `grep -n "npm publish" .github/workflows/ci.yml | grep -v -- "--dry-run"` prints nothing. Validate the YAML. Stop if anything fails.
...
- [ ] T061 Dry runs 2 and 3:
  1. **Run 2, everything green.** Remove the failure step, set `VERSION` to `1.4.0-dev.2`, repeat the safety check, commit, push, then tag and push `v1.4.0-dev.2`. Every job succeeds:
     ...
     - the run shows a deployment to the `pypi` environment, **if** it exists by then. If it doesn't, the job fails at environment protection: record that, and rerun after T047.
...
- [ ] T066 (**irreversible**; needs the user's go-ahead to release) Push tag `v1.4.0` on the merge commit, only if all of these hold:
  - `main` is green;
  - `VERSION` is `1.4.0`;
  - the GitHub environment `pypi` exists and the user confirms the PyPI pending publisher (T047);
...
- **US3 (T042–T047)**: T042 needs the scripts from T030, T031, T048 and T052 to exist before CI can pass, so run it after US4's scripts, or accept a red PR run until they land. T043 → T045 → T046. T044 is independent. T047 can happen at any time.
...
1. + US2: 521/521 on five Python builds, and thread-safe. The port is now releasable in principle.
