...
- [ ] T007 [P] Create `rust/scripts/set-version.sh` (bash, `set -euo pipefail`): read `../VERSION` (trimmed),
  rewrite the first `version = "…"` line of `Cargo.toml`, and run `cargo update -p persian-text-guard
  --offline` so the lock file follows; print the old and new versions, or "no change". **Self-check**:
  with `VERSION` unchanged it reports "no change"; a hand-edited `version = "0.0.1"` in `Cargo.toml` makes
  `cargo build` fail with the message from T004; running the script restores the version and the build.
...
- [ ] T013 [P] Create `rust/src/types.rs` and `rust/src/errors.rs` exactly as
  ...
  - `BannedWord` with `new`, `with_mode`, `with_category`; `ProfanityFilterOptions` (`#[non_exhaustive]`,
    `Default` all `true`, three setters); `ProfanityMatch<'f>` with `range()`;
...
- [ ] T017 [P] Port `js/src/fold.ts` to `rust/src/fold.rs`: the fold, squeeze and character-class helpers on
  ...
  letters for U+00C0–U+024F come from `nfd` at first use through `LazyLock`, or a table generated by T010's
  program if simpler; record the choice in the module docs).
...
- [ ] T034 [US2] Create `rust/tests/corpus/evaluate.rs`, a port of `js/test/corpus/evaluate.ts` against the
  ...
  matching kinds return `containsProfanity`, `firstMatch`, `matches` (positions converted from bytes to
  code points for comparison, or the expected converted to bytes: pick one and state it) and `censored`,
...
- [ ] T037 [US2] Create `rust/tests/no_panic.rs` with `proptest` (SC-007, G1, G9): strategies for arbitrary
  ...
  and runs of repeats, and for arbitrary `Vec<u8>` (mostly invalid UTF-8); 100,000 cases each (`ProptestConfig
  { cases: 100_000, .. }`, run in release-like `opt-level` via `[profile.test] opt-level = 1` if needed for
  time); every public text function called with the default filter; for bytes, the byte versions agree with
  the string versions on `String::from_utf8_lossy`, and every region is inside the input. Record the run
  time.
- [ ] T038 [US2] Create `rust/tests/threads.rs` (SC-008, G6, G7): one filter per corpus configuration, a
  ...
  twice and comparing; and 8 threads calling `WordList::all()` at the same moment in a fresh process (the test
  re-runs its own test binary with an argument, or a small `examples/` program), all getting the same pointer.
...
- [ ] T042 [US3] **Do this after T048 and T052**, whose scripts the jobs run. Add the Rust jobs to
  ...
  - **Job `rust-checks`**, `name: Rust checks`, ubuntu, stable with `components: rustfmt, clippy`,
    ...
    (T048); `actions/upload-artifact@v4` with `name: crate-package`, `path: rust/target/package/*.crate`.
...
- [ ] T048 [P] [US4] Create `rust/bench/` (`persian-text-guard-bench`, `publish = false`, not a workspace
  member, its own lock file; `criterion = "0.8"` and `persian-text-guard = { path = ".." }`):
  ...
  and CPU model; `src/bin/bench_gate.rs` running `cargo bench --bench filter -- CleanShortMessage
  --warm-up-time 1 --measurement-time 3` itself or reading its result, failing when the mean exceeds 50 µs
  (`--limit-us` overrides; `--limit-us 5` applies SC-005).
- [ ] T049 [US4] Write the full `rust/README.md` (FR-024, research R13), every code block a `rust` doc test:
  ...
  - **English**: installation (`cargo add persian-text-guard`, minimum Rust 1.85); the quick start; what
    ...
    raw bytes (the byte versions, invalid UTF-8 kept in the output); matching on `#[non_exhaustive]` enums
...
- [ ] T051 [US4] Confirm the README examples are doc tests: `cargo test --locked --doc` lists at least 10 README
  blocks (from `lib.rs`'s crate docs) plus one per public type, and all pass on stable and 1.85. Record the
  count.
