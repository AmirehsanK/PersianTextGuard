...
- [X] T010 Create `rust/tools/gen_tables.cs`, a .NET 10 file-based program (no project file), with a header
...
- [X] T011 [P] Create `rust/src/unicode.rs`, the port of `js/src/unicode.ts` using the porting table:
...
- [X] T012 [P] Create unit tests in `rust/src/unicode.rs` (`#[cfg(test)] mod tests`), porting
...
- [X] T013 [P] Create `rust/src/types.rs` and `rust/src/errors.rs` exactly as
...
- [X] T014 [P] Create `rust/src/utf16.rs` (research R2): `to_view(&str) -> Cow<[u16]>`-like helper (a `Vec<u16>`
...
- [X] T015 Port `js/src/normalizer.ts` to `rust/src/normalizer.rs`: the step bits, presets, `normalize_with_map`
...
- [X] T016 Port `js/src/word-list.ts` to `rust/src/word_list.rs`: `WordList` with `all()` and
...
- [X] T017 [P] Port `js/src/fold.ts` to `rust/src/fold.rs`: the fold, squeeze and character-class helpers on
...
- [X] T018 Port `js/src/source-map.ts` to `rust/src/source_map.rs`: `MappedText { text, start_map, end_map }`,
  `ReadingKind`, `build` with a per-message cache of four slots, `chunk_map`, `whole_message_map`.
- [X] T019 Port `js/test/internals.test.ts` to unit tests in `rust/src/normalizer.rs`,
...
- [X] T020 Run the lint command from T008 and `cargo test --locked` (unit tests) on stable and 1.85, and
